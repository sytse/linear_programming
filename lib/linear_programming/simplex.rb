module LinearProgramming
  module Simplex
    ...
    module GeneralLinearProgrammingProblem
      def solved?
        @starred_row_indices ||= each_with_index.each_with_object([]) do |(value, row, column), starred_row_indices|
          starred_row_indices << row if value.negative? && (column >= free_variable_size)
        end
    ...
    # Phase 2
    module StandardMaximizationProblem
      def solved?
        objective.none?(&:negative?)
      end
    ...
    def solve
      ...
      Array.new(objective.size) do |column_index|
        if (row_index = @active_variables.index(column_index)) && @rows.fetch(row_index).fetch(column_index).positive?
    ...
    def test_ratios
      constraints.map do |constraint|
        ...
        constraint.last / denominator if denominator.positive?
      end
    end

    def lowest_test_ratio_row_indices
      raise Problem::Unbounded unless lowest_test_ratio = test_ratios.compact.min
